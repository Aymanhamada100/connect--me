...
function CompanyLogo({ domain, company }) {
    ...
    return (
        <div className="w-11 h-11 rounded-xl border border-gray-100 bg-white flex items-center justify-center shrink-0 overflow-hidden p-1">
            <img src={faviconUrl} alt={company} className="w-full h-full object-contain" onError={() => setError(true)} />
        </div>
    );
}
...
function Toast({ message, onClose }) {
    ...
    return (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-gray-900 text-white text-sm font-semibold px-6 py-3 rounded-full shadow-lg flex items-center gap-3 animate-bounce-once">
            <span>🔒</span>
            {message}
            <button onClick={onClose} className="ml-2 text-gray-400 hover:text-white transition-colors">✕</button>
        </div>
    );
}
    ...
    return (
        <div className="bg-white rounded-2xl p-5 border border-gray-200 hover:border-orange-400 hover:shadow-lg transition-all duration-200 cursor-pointer">
            ...
            <div className="flex flex-col gap-1 mb-4">
                <div className="flex items-center gap-2 text-xs text-gray-500"><span>📍</span> {job.location}</div>
                <div className="flex items-center gap-2 text-xs text-gray-500"><span>💼</span> {job.type}</div>
                <div className="flex items-center gap-2 text-xs text-gray-500"><span>💰</span> {job.salary}</div>
            </div>

            <div className="flex flex-wrap gap-2">
                <span className="bg-gray-100 text-gray-600 text-xs font-semibold px-3 py-1 rounded-full border border-gray-200">Remote</span>
                <span className="bg-gray-100 text-gray-600 text-xs font-semibold px-3 py-1 rounded-full border border-gray-200">Priority slots available</span>
                <span className="bg-gray-900 text-white text-xs font-semibold px-3 py-1 rounded-full flex items-center gap-1">🕐 {job.posted}</span>
            </div>
        </div>
    );
}
...
function Modal({ open, onClose, jobs }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white rounded-2xl w-full max-w-xl max-h-[80vh] flex flex-col p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xl font-extrabold text-gray-900">All <span className="text-orange-500">Jobs</span></h3>
                ...
                <div className="overflow-y-auto flex flex-col gap-3 pr-1">
                    {filtered.length === 0 ? (
                        ...
                        filtered.map((j) => (
                            <div key={j.id} className="flex items-center gap-3 p-3 border border-gray-100 rounded-xl hover:border-orange-400 hover:bg-orange-50 transition-all cursor-pointer">
                                ...
                                <div className="flex flex-col items-end gap-1">
                                    <span className="bg-gray-100 text-gray-600 text-xs font-semibold px-2 py-0.5 rounded-full">{j.type}</span>
                                    <span className="bg-gray-900 text-white text-xs font-semibold px-2 py-0.5 rounded-full">{j.posted}</span>
                                </div>
                            </div>
                        ))
                    )}
...
export default function HeroThree() {
    ...
    const toggleSave = (id) => {
        if (!getToken()) {
            setToast("Please sign in to save jobs");
            return;
        }
        setSaved((prev) => {
