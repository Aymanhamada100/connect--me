...
// ─── FIELDS ───────────────────────────────────────────────────────────────────
const profileFields = [
  { key: "companyFocus", label: "Company Focus", icon: "🎯", link: false },
  { key: "industry", label: "Industry", icon: "🏭", link: false },
  { key: "companySize", label: "Company Size", icon: "👥", link: false },
  { key: "companyAddress", label: "Address", icon: "📍", link: false },
  { key: "phoneNumber", label: "Phone Number", icon: "📞", link: false },
  { key: "officialEmail", label: "Official Email", icon: "📧", link: false },
  { key: "websiteLink", label: "Website", icon: "🌐", link: true },
  { key: "socialMediaLinks", label: "Social Media", icon: "🔗", link: true },
];
...
export default function ProfilePageCompany() {
  ...
  useEffect(() => {
    ...
    if (!token || !companyId) {
      console.warn("No token or companyId found in storage");
      setLoading(false);
      return;
    }
    (async () => {
      setLoading(true);
      try {
        const url = APICompany.profile(companyId);
        ...
        const profileData =
          ...
          data?.company ||  
          data?.profile || 
          (data?.companyName ? data : null);      

        if (profileData) {
          setProfile(profileData);
          setDraft(profileData);
        } else {
          console.error("Unknown response shape:", data);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // يتعامل مع كل أشكال الـ response
      const updated =
        data?.data ||
        data?.company ||
        data?.profile ||
      ...
      if (updated) {
        // السيرفر رجّع البيانات المحدثة
        setProfile(updated);
        setDraft(updated);
      } else {
        // السيرفر رجّع success بس من غير data — نحتفظ بالـ draft
        setProfile(draft);
      }
  ...
  const location = profile
    ? [...new Set([profile.country, profile.city, profile.location].filter(Boolean))].join(", ") || "—"
    : "—";
        ...
        <aside className="w-60 shrink-0 bg-white border-r border-slate-100 flex flex-col py-7 px-4">
          ...
          {profile && (
            <div className="mx-1 mb-6 bg-linear-to-br from-blue-600 to-blue-700 rounded-2xl p-4 text-white">
              ...
              <p className="font-bold text-sm leading-tight truncate">{profile.companyName}</p>
              <p className="text-blue-200 text-xs mt-0.5 truncate">{profile.industry || "Company"}</p>
            </div>
          )}
        ...
        <div className="flex flex-col flex-1 min-w-0 bg-[#F4F7FC]">
          ...
          <div className="flex items-center gap-3 px-8 py-5 bg-white border-b border-slate-100">
            <div className="flex-1">
              ...
              <p className="text-slate-400 text-xs mt-0.5">Manage your company information</p>
            </div>
                ...
                <>
                  ...
                  <div className="bg-white rounded-3xl shadow-sm p-6 flex items-center gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className="text-xl font-black text-slate-900">{profile.companyName}</h2>
                      <p className="text-blue-600 text-sm font-semibold mt-0.5">Company</p>
                      {location !== "—" && (
